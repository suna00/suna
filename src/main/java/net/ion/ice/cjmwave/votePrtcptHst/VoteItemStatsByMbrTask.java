...
public class VoteItemStatsByMbrTask {
    ...
    // full statistics
    // 성별
    public void execVoteItemStatsBySexAll() {

        logger.info("start statistics task - execVoteItemStatsBySexAll");
        ...
        for (Node voteBasInfo : voteBasInfoList) {
            ...
            Integer startHstSeq = 0;
            List<Map<String,Object>> voteItemStatsBySexList = selectItemStatsBySexListAll(voteBasInfo.getId());
            List<Map<String,Object>> insertItemStatsBySexList = new ArrayList<>();
            Integer totalVoteNum = 0;
            for (Map voteItemStatsBySex : voteItemStatsBySexList) {
                Integer voteNum =
        ...
        logger.info("complete statistics task - execVoteItemStatsBySexAll");
    }


    private List<Map<String,Object>> selectItemStatsBySexListAll(String voteSeq) {
        String selectQuery = "SELECT voteSeq, voteItemSeq, sexCd, sum(voteNum) AS voteNum, voteDate " +
                "FROM (" +
                "  SELECT" +
                "    voteSeq, voteItemSeq, mbrId, v.snsTypeCd, v.snsKey, sexCd, cntryCd, voteNum, hstSeq, v.voteDate, v.created " +
                "  FROM voteItemStatsHstByMbr v LEFT JOIN mbrInfo m " +
                "  ON v.snsTypeCd = m.snsTypeCd AND v.snsKey = m.snsKey) t " +
                "WHERE voteSeq=? AND sexCd IS NOT NULL AND cntryCd IS NOT NULL " +
                "GROUP BY voteDate, voteSeq, voteItemSeq, sexCd ";
        return jdbcTemplate.queryForList(selectQuery, voteSeq);
    }

    // full statistics
    // 국가별.
    public void execVoteItemStatsByCntryAll() {

        logger.info("start statistics task - execVoteItemStatsByCntryAll");
        ...
        for (Node voteBasInfo : voteBasInfoList) {
            ...
            Integer startHstSeq = 0;
            List<Map<String,Object>> voteItemStatsByCntryList = selectItemStatsByCntryListAll(voteBasInfo.getId());
            List<Map<String,Object>> insertItemStatsByCntryList = new ArrayList<>();
            Integer totalVoteNum = 0;
            for (Map voteItemStatsByCntry : voteItemStatsByCntryList) {
        ...
        logger.info("complete statistics task - execVoteItemStatsByCntryAll");
    }


    private List<Map<String,Object>> selectItemStatsByCntryListAll(String voteSeq) {
